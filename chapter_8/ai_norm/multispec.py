...
def run(norm_spec_q):
    logger = logging_setup.get_logger(
        config.LOGGER_NAME_MULTISPEC, config.LOGGING_LEVEL_MULTISPEC
    )
    ...
    while True:
        ...
        if overtime > config.CLOCK_PERIOD_SPEC:
            ...
            running_behind = True
        frequencies = None
        while not norm_spec_q.empty():
            frequencies, magnitudes = norm_spec_q.get()
...
class Frame:
    ...
    def _initialize_frame(self, frequencies, magnitudes):
        ...
        self.img = self.ax.imshow(
            self.img_data,
            aspect="auto",
            cmap=config.MULTISPEC_VIZ_CMAP,
