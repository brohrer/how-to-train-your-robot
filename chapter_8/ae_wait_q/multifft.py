...
def run(listen_fft_q, fft_freq_q, fft_spec_q):
    ...
    n_samples = int(
        config.FFT_WINDOW_LENGTH_LONG * config.SAMPLING_RATE / 1000
    )
    ...
    while True:
        ...
        freqs, mags = time_to_freq(
            sample_window, config.FFT_WINDOW_LENGTH_SHORT
        )
...
def time_to_freq(
    ...
    n_samples = np.minimum(n_samples, timeseries_uncropped.size)
    timeseries = timeseries_uncropped[-n_samples:]
