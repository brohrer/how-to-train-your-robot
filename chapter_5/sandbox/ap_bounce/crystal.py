...
class Crystal:
    def __init__(self, init_dict):
        ...
        # self.internal_damping = np.zeros((self.n_atoms, self.n_atoms))

        # Handle the case where there is no internal damping provided.
        # try:
        #     internal_damping_coef = init_dict["internal_damping"]
        #     # self.internal_damping = init_dict["internal_damping"]
        # except KeyError:
        #     internal_damping_coef = 0
        #     # self.internal_damping = 0
        ...
        for i_conn in range(n_connections):
            ...
            # self.internal_damping[i_atom, j_atom] = internal_damping_coef
            # self.internal_damping[j_atom, i_atom] = internal_damping_coef
        ...
        self.dx_internal = np.zeros((self.n_atoms, self.n_atoms))
        self.dy_internal = np.zeros((self.n_atoms, self.n_atoms))
...
def internal_interactions_numba(
    ...
    # alpha = ALPHA
    # f_mag = np.sign(compression) * (
    #     k_internal * (1 - np.exp(
    #         -np.abs(compression) / (l_connection * alpha)
    #     ))
    # )
...
def external_interactions_numba(
    ...
    # alpha = ALPHA
    # f_mag = k_ab * (1 - np.exp( -compression / (r_ab * alpha)))
...
def wall_forces_numba(
    ...
    # alpha = ALPHA
    # f_total = k_external * (1 - np.exp( -compression / (r_atoms * alpha)))
...
def update_positions_numba(
    ...
    vx = np.sign(vx) * (1 - np.exp( -np.abs(vx) / SPEED_LIMIT))
    vy = np.sign(vy) * (1 - np.exp( -np.abs(vy) / SPEED_LIMIT))
