...
class Simulation:
    def __init__(self):
        ...
        self.add_wall(
            ...
            # y_left=0.0,
            y_left=config.FLOOR_DROP + config.FLOOR_OFFSET,
            x_right=0.0,
            y_right=config.FLOOR_OFFSET,
        )
    ...
    def disc_wall_interactions(self):
        ...
        self.fx_wall_disc += f_total * self.x_n / np.abs(distance)
        self.fy_wall_disc += f_total * self.y_n / np.abs(distance)
    ...
    def update_positions(self):
        ...
        # Wrap around side to side
        i_left = np.where(self.x < 0)
        self.x[i_left] += config.FIG_WIDTH
        self.y[i_left] += config.FLOOR_DROP + np.random.sample()
        i_right = np.where(self.x > config.FIG_WIDTH)
        self.x[i_right] -= config.FIG_WIDTH
        self.y[i_right] -= config.FLOOR_DROP
