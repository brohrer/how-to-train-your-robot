...
class Simulation:
    ...
    def disc_disc_interactions(self):
        # dx = np.tile(self.x[np.newaxis, :], (self.n_discs, 1)) - np.tile(
        #     self.x[:, np.newaxis], (1, self.n_discs)
        # )
        dx = np.matmul(self.row_tile, self.x[np.newaxis, :]) - np.matmul(
            self.x[:, np.newaxis], self.col_tile
        )
        # dy = np.tile(self.y[np.newaxis, :], (self.n_discs, 1)) - np.tile(
        #     self.y[:, np.newaxis], (1, self.n_discs)
        # )
        dy = np.matmul(self.row_tile, self.y[np.newaxis, :]) - np.matmul(
            self.y[:, np.newaxis], self.col_tile
        )
        epsilon = 1e-12
        distance = (dx**2 + dy**2) ** 0.5 + epsilon
        compression = self.r_disc_disc - distance
        compression = np.maximum(compression, 0)

        # This is the longer, clearer version.
        # It was reformulated below to save
        # some element-wise matrix multiplication.
        # angle_of_action = np.arctan2(dy, dx)
        # sin_theta = dy / distance
        # cos_theta = dx / distance
        # self.fx_disc_disc += f_mag * cos_theta
        # self.fy_disc_disc += f_mag * sin_theta

        f_mag = self.k_disc_disc * compression
        f_norm = f_mag / distance
        self.fx_disc_disc += f_norm * dx
        self.fy_disc_disc += f_norm * dy
    ...
    def calculate_forces(self):
        self.fx_disc = 0.0
        self.fy_disc = 0.0
        self.fx_disc_disc = 0.0
        self.fy_disc_disc = 0.0
