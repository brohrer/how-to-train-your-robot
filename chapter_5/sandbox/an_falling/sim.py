...
def run(q):
    ...
    while True:
        ...
        if steps_since_q_update >= steps_per_q_update:
            logger.debug(json.dumps({"ts": 0, "state": 0}))
...
class Simulation:
    def __init__(self):
        ...
        self.x = np.array(self.x, dtype=np.half)
        self.y = np.array(self.y, dtype=np.half)
        self.x_n = np.array(self.x_n, dtype=np.half)[:, np.newaxis]
        self.y_n = np.array(self.y_n, dtype=np.half)[:, np.newaxis]
        self.x_wall = np.array(self.x_wall, dtype=np.half)
        self.y_wall = np.array(self.y_wall, dtype=np.half)
        self.vx = np.array(self.vx, dtype=np.half)
        self.vy = np.array(self.vy, dtype=np.half)
        self.r = np.array(self.r, dtype=np.half)
        self.k = np.array(self.k, dtype=np.half)
        self.m = np.array(self.m, dtype=np.half)
        self.fx_disc = np.zeros(self.n_discs, dtype=np.half)
        self.fy_disc = np.zeros(self.n_discs, dtype=np.half)
        self.fx_disc_disc = np.zeros(
            (self.n_discs, self.n_discs), dtype=np.half
        )
        self.fy_disc_disc = np.zeros(
            (self.n_discs, self.n_discs), dtype=np.half
        )
        self.fx_wall_disc = np.zeros(
            (self.n_walls, self.n_discs), dtype=np.half
        )
        self.fy_wall_disc = np.zeros(
            (self.n_walls, self.n_discs), dtype=np.half
        )
        ...
        self.row_tile = np.ones((self.n_discs, 1), dtype=np.half)
        self.col_tile = np.ones((1, self.n_discs), dtype=np.half)
    ...
    def disc_wall_interactions(self):
        ...
        f_total = self.k[np.newaxis, :] * compression
        ...
        epsilon = 1e-6
        self.fx_wall_disc += f_total * self.x_n / (np.abs(distance) + epsilon)
        self.fy_wall_disc += f_total * self.y_n / (np.abs(distance) + epsilon)
    ...
    def update_positions(self):
        ...
        i_bottom = np.where(
            self.y[: self.i_first_feature] < -config.PEG_RADIUS
        )
        self.y[i_bottom] += config.FIG_HEIGHT + 2 * config.PEG_RADIUS
