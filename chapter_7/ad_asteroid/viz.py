...
class Frame:
    ...
    def update(self, viz_info):
        ...
        for body_id in viz_info.keys():
            body_path, color = self.calculate_body_path(viz_info[body_id])
            if body_id not in self.body_patches:
                self.body_patches[body_id] = self.draw_body(body_path, color)
            else:
    ...
    def calculate_body_path(self, body_viz_info):
        ...
        color = config.BODY_COLOR[body_type]
        ...
        return body_path, color

    def draw_body(self, body_path, color):
